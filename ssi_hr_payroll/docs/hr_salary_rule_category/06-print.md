...
- **Record:** The salary rule category record to print already exists.
- **Config:** At least one `print_document_type` is configured for the
  `hr.salary_rule_category` model with a report linked to it. Without this, the wizard
...
2. Open the salary rule category record to print.
