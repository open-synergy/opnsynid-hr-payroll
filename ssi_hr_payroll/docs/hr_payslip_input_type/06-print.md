...
- **Record:** The payslip input type record to print already exists.
- **Config:** At least one `print_document_type` is configured for the
  `hr.payslip_input_type` model with a report linked to it. Without this, the wizard
...
2. Open the payslip input type record to print.
