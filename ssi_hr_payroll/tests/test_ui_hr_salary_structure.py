...
class TestUiHrSalaryStructure(HttpSavepointCase):
    """UI tour test suite for the ``hr.salary_structure`` menu."""

    @classmethod
    def setUpClass(cls):
        """Grant the configurator group to admin and prepare print data."""
        super().setUpClass()
        # Pre-Condition: the Salary Structures master data menu is gated by the
        # Salary Structure configurator group. Without it the tour would fail on
        # its very first step because the menu is not rendered at all.
        cls.user_admin = cls.env.ref("base.user_admin")
        cls.group_configurator = cls.env.ref(
            "ssi_hr_payroll.hr_salary_structure_configurator_group"
        )
        ...
        # Pre-Condition for Generate Code (docs/hr_salary_structure/01-create.md
        ...
        cls.code_sequence = cls.env["ir.sequence"].create(
            {
                "name": "TOUR Salary Structure Code Sequence",
                "code": "ssi_hr_payroll.tour.hr_salary_structure",
                "prefix": "TOURSEQSS",
                "padding": 4,
            }
        )
        cls.code_sequence_template = cls.env["sequence.template"].create(
            {
                "name": "TOUR Salary Structure Sequence Template",
                "model_id": cls.env["ir.model"]._get_id("hr.salary_structure"),
                "sequence_field_id": cls.env["ir.model.fields"]
                ._get("hr.salary_structure", "code")
                .id,
                "date_field_id": cls.env["ir.model.fields"]
                ._get("hr.salary_structure", "create_date")
                .id,
        ...
        # linking a report to ``hr.salary_structure`` is required for the
        ...
        cls.print_report_action = cls.env["ir.actions.report"].create(
            {
                "name": "TOUR Salary Structure Report",
                "model": "hr.salary_structure",
                "report_type": "qweb-pdf",
                "report_name": "ssi_hr_payroll.tour_salary_structure_report",
            }
        )
        ...
        cls.salary_structure_print = cls.env["hr.salary_structure"].create(
            {
                "name": "TOUR PRINT SALARY STRUCTURE",
                "code": "TOURPRNSS",
            }
        )
