...
class TestUiHrPayslipType(HttpSavepointCase):
    """UI tour test suite for the ``hr.payslip_type`` menu."""

    @classmethod
    def setUpClass(cls):
        """Grant the configurator group to admin and prepare print data."""
        super().setUpClass()
        # Pre-Condition: the Payslip Type master data menu is gated by the
        # Payslip Type configurator group. Without it the tour would fail on
        # its very first step because the menu is not rendered at all.
        cls.user_admin = cls.env.ref("base.user_admin")
        cls.group_configurator = cls.env.ref(
            "ssi_hr_payroll.hr_payslip_type_configurator_group"
        )
        ...
        # Pre-Condition for Generate Code (docs/hr_payslip_type/01-create.md
        ...
        cls.code_sequence = cls.env["ir.sequence"].create(
            {
                "name": "TOUR Payslip Type Code Sequence",
                "code": "ssi_hr_payroll.tour.hr_payslip_type",
                "prefix": "TOURSEQPT",
                "padding": 4,
            }
        )
        cls.code_sequence_template = cls.env["sequence.template"].create(
            {
                "name": "TOUR Payslip Type Sequence Template",
                "model_id": cls.env["ir.model"]._get_id("hr.payslip_type"),
                "sequence_field_id": cls.env["ir.model.fields"]
                ._get("hr.payslip_type", "code")
                .id,
                "date_field_id": cls.env["ir.model.fields"]
                ._get("hr.payslip_type", "create_date")
                .id,
        ...
        # linking a report to ``hr.payslip_type`` is required for the
        ...
        cls.print_report_action = cls.env["ir.actions.report"].create(
            {
                "name": "TOUR Payslip Type Report",
                "model": "hr.payslip_type",
                "report_type": "qweb-pdf",
                "report_name": "ssi_hr_payroll.tour_payslip_type_report",
            }
        )
        ...
        cls.payslip_type_print = cls.env["hr.payslip_type"].create(
            {
                "name": "TOUR PRINT PAYSLIP TYPE",
                "code": "TOURPRNPT",
            }
        )
    ...
    def test_create(self):
        ...
        self.start_tour("/web", "ssi_hr_payroll_hr_payslip_type_create", login="admin")

    def test_print(self):
        """Assert the Print wizard opens then close it, without printing.

        IK: docs/hr_payslip_type/06-print.md
        ...
        self.start_tour(
            "/web",
            "ssi_hr_payroll_hr_payslip_type_print",
            login="admin",
        )
