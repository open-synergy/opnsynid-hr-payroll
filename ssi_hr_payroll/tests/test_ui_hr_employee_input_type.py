...
class TestUiHrEmployeeInputType(HttpSavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.group_configurator = cls.env.ref(
            "ssi_hr_payroll.hr_employee_input_type_configurator_group"
        )
        ...
        # Pre-Condition for Generate Code (docs/hr_employee_input_type/01-create.md
        ...
        cls.code_sequence = cls.env["ir.sequence"].create(
            {
                "name": "TOUR Employee Input Type Code Sequence",
                "code": "ssi_hr_payroll.tour.hr_employee_input_type",
                "prefix": "TOURSEQEIT",
                "padding": 4,
            }
        )
        cls.code_sequence_template = cls.env["sequence.template"].create(
            {
                "name": "TOUR Employee Input Type Sequence Template",
                "model_id": cls.env["ir.model"]._get_id("hr.employee_input_type"),
                "sequence_field_id": cls.env["ir.model.fields"]
                ._get("hr.employee_input_type", "code")
                .id,
                "date_field_id": cls.env["ir.model.fields"]
                ._get("hr.employee_input_type", "create_date")
                .id,
        ...
        # linking a report to ``hr.employee_input_type`` is required for the
        ...
        cls.print_report_action = cls.env["ir.actions.report"].create(
            {
                "name": "TOUR Employee Input Type Report",
                "model": "hr.employee_input_type",
                "report_type": "qweb-pdf",
                "report_name": "ssi_hr_payroll.tour_employee_input_type_report",
            }
        )
        ...
        cls.employee_input_type_print = cls.env["hr.employee_input_type"].create(
            {
                "name": "TOUR PRINT EMPLOYEE INPUT TYPE",
                "code": "TOURPRNEIT",
            }
        )
    ...
    def test_create(self):
        ...
        self.start_tour(
            "/web", "ssi_hr_payroll_hr_employee_input_type_create", login="admin"
        )

    def test_print(self):
        """Assert the Print wizard opens then close it, without printing.

        IK: docs/hr_employee_input_type/06-print.md
        ...
        self.start_tour(
            "/web",
            "ssi_hr_payroll_hr_employee_input_type_print",
            login="admin",
        )
