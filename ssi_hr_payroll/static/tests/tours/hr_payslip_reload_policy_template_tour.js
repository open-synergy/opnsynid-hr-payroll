odoo.define("ssi_hr_payroll.hr_payslip_reload_policy_template_tour", function (
    require
) {
    "use strict";
    ...
    tour.register(
        ...
        [
            ...
            // ── Flow 2 — Open the payslip whose policy template needs to be
            // reloaded. The Pre-Condition record (any status, admin is a
            // member of `base.group_system` by default) is prepared in
            // setUpClass for employee "TOUR PAYSLIP RELOAD POLICY".
            {
                content: "Open the payslip",
                trigger:
                    ".o_data_row:contains(TOUR PAYSLIP RELOAD POLICY) .o_data_cell:first",
                extra_trigger: ".o_list_view",
            },
            {
            ...
            {
                content: "The Reload Template Policy button is displayed and enabled",
            ...
            // ── Post-Condition — the Policies tab remains displayed and no
            // error dialog was raised. The resulting `policy_template_id`
            // value (and the dependent `*_ok` fields it recomputes) is out
            // of tour scope — see the Boundary note in the docstring and
            // 16-reload-policy-template.md; that is unit test territory.
            {
                content: "The Policies tab is still displayed, with no error raised",
