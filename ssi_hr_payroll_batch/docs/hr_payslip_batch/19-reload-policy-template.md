# Reload Policy Template of Employee Payslip Batch
...
- **Record:** An existing batch is open, in any status.
- **Config:** An active `policy.template` for `hr.payslip_batch` is configured. If no
  template matches this record, clicking the button still succeeds but leaves **Policy
  Template** empty — a silent failure the actor should watch for.
...
2. Open the batch whose policy template needs to be reloaded.
...
- The **Policy Template** field is reloaded with the template that currently matches the
  record.
- The policy fields below it recompute from the reloaded template.
