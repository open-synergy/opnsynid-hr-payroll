odoo.define("ssi_hr_payroll_batch.hr_payslip_batch_restart_approval_tour", function (
    require
) {
    "use strict";
    ...
    tour.register(
        ...
        [
            ...
            // ── Flow 2 — Open the batch whose approval process is
            // stalled. The Pre-Condition record (Waiting for Approval,
            // without an approval template assigned) is prepared in
            // setUpClass for the type "TOUR BATCH RESTART APPROVAL".
            {
                content: "Open the batch",
                trigger:
                    ".o_data_row:contains(TOUR BATCH RESTART APPROVAL) .o_data_cell:first",
                extra_trigger: ".o_list_view",
            },
            {
            ...
            // and the Approvals tab displays the approval list newly
            // formed from the approval template. Row count/content is
            // out of tour scope (unit test territory); the gate below
            // only proves the reload happened — the Pre-Condition
            // record has no approval template and no approval line at
            // all, so a row cannot appear here unless the restart
            // actually ran (patterns.md §P).
            {
